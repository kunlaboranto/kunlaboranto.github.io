...
(
    SELECT SYSDATE
         ...
         , CASE WHEN S.SQL_ID IS NULL 
                THEN NULL
                ELSE (SELECT SQL_CHILD_NUMBER||'' FROM V$SESSION S1 WHERE S1.SQL_ID = SA.SQL_ID AND ROWNUM = 1 )
            END CHILD_NUMBER
         --, LENGTHB( SA.SQL_TEXT ) AS LEN
         --, LPAD( LENGTHB( SA.SQL_TEXT ), 4) ||'> '|| RPAD(REPLACE(REPLACE(REPLACE(REPLACE( SA.SQL_TEXT, CHR(10), ' '),CHR(9),' '),CHR(9),' '),'  ',' '),160)||CHR(10) QRY
         , RPAD(REPLACE(REPLACE(REPLACE(REPLACE( SA.SQL_TEXT, CHR(10), ' '),CHR(9),' '),CHR(9),' '),'  ',' '),160)||CHR(10) QRY
      FROM V$SQLAREA SA
           LEFT OUTER JOIN ( SELECT DISTINCT SQL_ID FROM V$SESSION WHERE STATUS in ( 'ACTIVE', 'RUNNING' ) ) S
                   ON S.SQL_ID = SA.SQL_ID
     WHERE 1=1
       AND SA.ELAPSED_TIME > 0
       AND SA.EXECUTIONS > 0
       --AND SA.MODULE NOT LIKE '%Orange%'
       --AND LENGTHB( SA.SQL_TEXT )  < 1024
