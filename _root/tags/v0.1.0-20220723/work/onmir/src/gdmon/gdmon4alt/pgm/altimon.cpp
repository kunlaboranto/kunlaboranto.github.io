...
/* bluetheme */
char CONNECTION_CHK_ENABLE[128];
char CONNECTION_CHK_LIMIT [8];
char CONNECTION_CHK_ACT   [1024];
...
void processChk()
{
    ...
    /*********************************************
     PID
    *********************************************/
    sprintf( _PCOM,  "ps -ef | grep \"%s/bin/altibase -p boot from\" | grep -v grep | grep %s | awk '{print $2}'", (char*)getenv("ALTIBASE_HOME"), UID);
    sprintf( _PCOM,  "ps -ef | grep \"%s/bin/altibase -p boot from\" | grep -v grep | grep \"^altibase\" | grep %s | awk '{print $2}'", (char*)getenv("ALTIBASE_HOME"), UID);
...
_log((char*)"REQUEST_TCP_LOG = %s\n", REQUEST);
    ...
    {
        sprintf(sndMsg, "%03d%s%03d%s%03d%s", strlen(gPID), gPID,
                                              strlen(gCPU), gCPU, 
                                              strlen(gVSZ), gVSZ);
    }
    ...
    {
        sprintf(sndMsg, "%03d%s%03d%s%03d%s",   strlen(SYSTEM_FREEMEM),  SYSTEM_FREEMEM,
                                                strlen(SYSTEM_FREESWAP), SYSTEM_FREESWAP,
                                                strlen(SYSTEM_IDLE),     SYSTEM_IDLE);
    }
...
int main(int argc, char *argv[])
{
    ...
    *************************************/
    ...
    /*************************************
      configFile은 어디에??
    *************************************/
    sprintf(configFile, "%s/conf/altimon.conf", (char*)getenv("ALTIBASE_HOME"));
    ...
    *************************************/
    ...
    /* bluethem */
    memset(CONNECTION_CHK_ENABLE, 0x00, sizeof(CONNECTION_CHK_ENABLE));
    memset(CONNECTION_CHK_LIMIT , 0x00, sizeof(CONNECTION_CHK_LIMIT ));
    memset(CONNECTION_CHK_ACT   , 0x00, sizeof(CONNECTION_CHK_ACT   ));
...
_log((char*)"DURATION = [%s]\n", duration);
    ...
    /* bluetheme */
    getSimpleParam((char*)"OS_QUERY_GROUP_SET", (char*)"CONNECTION_CHK_ENABLE",   CONNECTION_CHK_ENABLE,   1);
    getSimpleParam((char*)"OS_QUERY_GROUP_SET", (char*)"CONNECTION_CHK_LIMIT" ,   CONNECTION_CHK_LIMIT ,   1);
    getSimpleParam((char*)"OS_QUERY_GROUP_SET", (char*)"CONNECTION_CHK_ACT"   ,   CONNECTION_CHK_ACT   ,   0);
    chkMemset(CONNECTION_CHK_ACT);
    ...
    *************************************/
    while (1)
    {
        ...
        {
            /* bluetheme */
            if (memcmp(CONNECTION_CHK_ENABLE, "ON", 2) == 0 )
            {
                sprintf( _PCOM,  "ps -ef | grep \"%s/bin/altibase -p boot from\" | grep -v grep | grep %s | awk '{print $2}'", 
                        (char*)getenv("ALTIBASE_HOME"), UID);
                memset( PID, 0x00, sizeof(PID));
                runCommand( _PCOM, PID);


                if ((int)strlen(PID) != 0 &&
                    atoi(CONNECTION_CHK_LIMIT) <= ++CONNECT_FAIL_CNT
                   )
                {
                    _log((char*)"[ALARM]:: [CONNECTION FAIL] Current (%d) >= Limit (%s)\n", CONNECT_FAIL_CNT, CONNECTION_CHK_LIMIT);
                    sprintf(tmpMsg, (char*)"[ALARM]:: [CONNECTION FAIL] Current (%d) >= Limit (%s)\n", CONNECT_FAIL_CNT, CONNECTION_CHK_LIMIT);
                    system(CONNECTION_CHK_ACT);
                    saveDB((char*)"1", (char*)"CONNECTION_CHK_ACT",  tmpMsg);

                    /* DB는 안 죽었으니까 이정도는 찍어줘야지... */
